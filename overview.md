...
- To run the tests, specify the **Run MATLAB Tests** task. License MATLAB for running the tests by mapping a secret variable to the `MLM_LICENSE_TOKEN` environment variable in the task. In this example, `myToken` is the name of the secret variable that holds the batch licensing token.
